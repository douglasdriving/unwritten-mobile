import { GenerateRandomString, TwoDaysFromNowInMs } from "../helpers/helpers"
import { GenerateRandomRoomArray, maxScenarioCount } from './dataGeneration.js';
...
export const GetFinishedStories = async () => {
  const finishedStories = rooms.filter(room => (room.finished));
  return finishedStories;
}

export const GetStoryKeys = async () => {
  const user = await GetUser();
  return user.storyKeys;
}
